...
describe('golangci-lint adapter', function()
  before_each(helpers.reset)
  after_each(helpers.reset)

  describe('parse()', function()
    it('decodes fixes into text edits', function()
    ...
    it('uses matching diagnostic ranges', function()
      ...
      local items =
        adapter.parse({ output = output, bufnr = bufnr, cwd = vim.fn.getcwd(), diagnostics = { diagnostic } })
      eq(items[1].range, helpers.range(0, 0, 0, 7))
      eq(items[1].action.title, 'message [tool]')
    end)

    it('ignores non-JSON and structurally invalid output', function()
    ...
    it('ignores other files and malformed text edits', function()
      local bufnr =
    ...
    for _, case in ipairs(malformed_edits) do
      it('rejects an entire suggestion containing ' .. case.name, function()
    ...
    it('preserves empty and null deletions and insertions at EOF', function()
