...
describe('Published code actions', function()
  before_each(helpers.reset)
  after_each(helpers.reset)

  describe('published actions', function()
    for _, form in ipairs({ 'changes', 'documentChanges' }) do
      it('applies multi-file ' .. form .. ' with unknown secondary versions', function()
    ...
    it('filters a selected edit by line and kind before applying it', function()
    ...
    it('an edit made after opening the menu invalidates both the listing and the selected edit', function()
      ...
      helpers.silence_prints()
      vim.lsp.util.apply_workspace_edit(found[1].edit, 'utf-8')
      eq(vim.api.nvim_buf_get_lines(bufnr, 0, -1, false), { 'changed after request' })
    end)

    it('replacing and clearing a source preserves the other sources', function()
    ...
    it('returns command-carrying actions unchanged', function()
      local actions = require('lint_actions')
    ...
    it('requires fresh publication after a buffer rename', function()
      local actions = require('lint_actions')
