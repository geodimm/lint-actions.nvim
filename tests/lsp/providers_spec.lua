...
describe('Provider code actions', function()
  before_each(helpers.reset)
  after_each(helpers.reset)

  describe('providers', function()
    it('answers requests from current buffer state without republishing', function()
    ...
    it('merges and sorts healthy sources even when one provider fails', function()
      local bufnr = helpers.new_buffer(vim.fs.joinpath(vim.fn.getcwd(), 'tests', 'provider-merge.txt'), { 'alpha' })
      helpers.silence_notifications()
    ...
    it('attaches to files opened after registration', function()
    ...
    it('stops a provider from answering', function()
      local bufnr =
        helpers.new_buffer(vim.fs.joinpath(vim.fn.getcwd(), 'tests', 'provider-unregister.txt'), { 'alpha' })
