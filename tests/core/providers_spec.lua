...
describe('Action providers', function()
  before_each(helpers.reset)
  after_each(helpers.reset)

  describe('register()', function()
    it('passes the requested range and kind filter to the provider', function()
    ...
    it('filters provider items by range and kind', function()
      local bufnr =
        helpers.new_buffer(vim.fs.joinpath(vim.fn.getcwd(), 'tests', 'provider-filter.txt'), { 'a', 'b', 'c' })
    ...
    it('restricts providers by filetype and by buffer', function()
    ...
    it('replaces a provider registered under the same source', function()
  ...
  describe('provider results are atomic', function()
    for _, failure in ipairs({ 'callback', 'validation', 'normalization', 'filtering' }) do
      it('discards the entire result on failure during ' .. failure, function()
    ...
    it('leaves buffers that are not ordinary files alone', function()
    ...
    it('validates its provider', function()
